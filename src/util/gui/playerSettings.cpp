...
void PlayerSettings::iniUI() {
    _rDialog = {windowSize.x / 2 - 250, windowSize.y / 2 - 200, 500, 600};
    // Title
    ...
    _txtb_sanity.set("Sanity", {_rDialog.x + 20, _rDialog.y + 80}, true);
...
std::set<MenuEntries> PlayerSettings::getUsableTowers() {
    std::set<MenuEntries> res;
    for (auto &entry: _usableTowers) {
        if(entry.isChecked())res.insert(entry.getMenuEntry());
    }
    return res;
}
